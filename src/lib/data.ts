...
// GET all
export async function dbGetAllSports(): Promise<Sport[]> { return db.sports; }
export async function dbGetAllAmenities(): Promise<Amenity[]> { return db.amenities; }
export async function dbGetAllFacilities(): Promise<Facility[]> { return db.facilities; }
export async function dbGetAllUsers(): Promise<UserProfile[]> { return db.users; }
export async function dbGetAllBookings(): Promise<Booking[]> { return db.bookings; }
export async function getAllEvents(): Promise<SportEvent[]> { return db.events; }
export async function getAllMembershipPlans(): Promise<MembershipPlan[]> { return db.membershipPlans; }
export async function getAllPricingRules(): Promise<PricingRule[]> { return db.pricingRules; }
export async function getAllPromotionRules(): Promise<PromotionRule[]> { return db.promotionRules; }
export async function getAllBlogPosts(): Promise<BlogPost[]> { return db.blogPosts; }
export async function getSiteSettings(): Promise<SiteSettings> { return db.siteSettings; }
...
// GET by ID
export async function getSportById(id: string): Promise<Sport | undefined> { return db.sports.find(s => s.id === id); }
export async function getAmenityById(id: string): Promise<Amenity | undefined> { return db.amenities.find(a => a.id === id); }
export async function dbGetFacilityById(id: string): Promise<Facility | undefined> { return db.facilities.find(f => f.id === id); }
export async function dbGetUserById(id: string): Promise<UserProfile | undefined> { return db.users.find(u => u.id === id); }
export async function dbGetBookingById(id: string): Promise<Booking | undefined> { return db.bookings.find(b => b.id === id); }
export async function getEventById(id: string): Promise<SportEvent | undefined> { return db.events.find(e => e.id === id); }
export async function getBlogPostBySlug(slug: string): Promise<BlogPost | undefined> { return db.blogPosts.find(p => p.slug === slug); }
export async function dbGetMembershipPlanById(id: string): Promise<MembershipPlan | undefined> { return db.membershipPlans.find(p => p.id === id); }
export async function dbGetTeamById(id: string): Promise<Team | undefined> { return db.teams.find(t => t.id === id); }
export async function getPricingRuleById(id: string): Promise<PricingRule | undefined> { return db.pricingRules.find(r => r.id === id); }
export async function getPromotionRuleByCode(code: string): Promise<PromotionRule | undefined> {
  return db.promotionRules.find(p => p.code?.toLowerCase() === code.toLowerCase() && p.isActive);
}
export async function dbGetSportById(id: string): Promise<Sport | undefined> { return db.sports.find(s => s.id === id); }

// GET by criteria
export async function dbGetFacilitiesByOwnerId(ownerId: string): Promise<Facility[]> {
  return db.facilities.filter(f => f.ownerId === ownerId);
}
export async function getPricingRulesByFacilityIds(facilityIds: string[]): Promise<PricingRule[]> {
    return db.pricingRules.filter(rule => 
...
export async function dbGetEventsByFacilityIds(facilityIds: string[]): Promise<SportEvent[]> {
    return db.events.filter(event => facilityIds.includes(event.facilityId));
}

export async function getLfgRequestsByFacilityIds(facilityIds: string[]): Promise<LfgRequest[]> {
    return db.lfgRequests.filter(req => facilityIds.includes(req.facilityId));
}

export async function getChallengesByFacilityIds(facilityIds: string[]): Promise<Challenge[]> {
    return db.challenges.filter(c => facilityIds.includes(c.facilityId));
}

export async function dbGetFacilitiesByIds(ids: string[]): Promise<Facility[]> {
  return db.facilities.filter(f => ids.includes(f.id));
}
...
export async function dbGetBookingsForFacilityOnDate(facilityId: string, date: string): Promise<Booking[]> {
  return db.bookings.filter(b => b.facilityId === facilityId && b.date === date && b.status === 'Confirmed');
}
...
export async function dbAddReview(data: Omit<Review, 'id' | 'createdAt' | 'userName' | 'userAvatar' | 'isPublicProfile'>): Promise<Review> {
    ...
    const facility = db.facilities.find(f => f.id === data.facilityId);
    if (facility) {
    ...
    const booking = db.bookings.find(b => b.id === data.bookingId);
    if (booking) {
...
export async function addPromotionRule(data: Omit<PromotionRule, 'id'>): Promise<PromotionRule> {
    ...
    db.promotionRules.push(newRule);
    saveData();
    return newRule;
}
...
export async function dbUpdateFacility(facilityData: any): Promise<Facility> {
    const index = db.facilities.findIndex(f => f.id === facilityData.id);
    if (index === -1) throw new Error("Facility not found");
    db.facilities[index] = { ...db.facilities[index], ...facilityData };
    saveData();
...
export async function dbUpdateSport(id: string, updates: Partial<Sport>): Promise<Sport> {
    const index = db.sports.findIndex(s => s.id === id);
...
export async function dbUpdateMembershipPlan(data: MembershipPlan): Promise<MembershipPlan> {
    const index = db.membershipPlans.findIndex(p => p.id === data.id);
    if (index === -1) throw new Error("Membership plan not found");
    db.membershipPlans[index] = { ...db.membershipPlans[index], ...data };
    saveData();
    return db.membershipPlans[index];
}
export async function updatePricingRule(data: PricingRule): Promise<PricingRule> {
    const index = db.pricingRules.findIndex(r => r.id === data.id);
    if (index === -1) throw new Error("Pricing rule not found");
    db.pricingRules[index] = { ...db.pricingRules[index], ...data };
    saveData();
    return db.pricingRules[index];
}
export async function updatePromotionRule(data: PromotionRule): Promise<PromotionRule> {
    const index = db.promotionRules.findIndex(r => r.id === data.id);
    if (index === -1) throw new Error("Promotion rule not found");
    db.promotionRules[index] = { ...db.promotionRules[index], ...data };
    saveData();
    return db.promotionRules[index];
}
...
export async function dbDeleteFacility(facilityId: string): Promise<void> {
  db.facilities = db.facilities.filter(f => f.id !== facilityId);
  db.bookings = db.bookings.filter(b => b.facilityId !== facilityId);
  saveData();
}
...
export async function deleteMembershipPlan(id: string): Promise<void> {
    db.membershipPlans = db.membershipPlans.filter(p => p.id !== id);
    saveData();
}
...
export async function deletePromotionRule(id: string): Promise<void> {
    db.promotionRules = db.promotionRules.filter(p => p.id !== id);
    saveData();
}
...
export async function updateSiteSettings(newSettings: Partial<SiteSettings>): Promise<SiteSettings> {
    db.siteSettings = { ...db.siteSettings, ...newSettings };
    saveData();
...
export async function dbGetOpenLfgRequests(): Promise<LfgRequest[]> {
    return db.lfgRequests.filter(r => r.status === 'open');
}
...
export async function calculateDynamicPrice(
  ...
  const applicableRules = db.pricingRules.filter(rule => {
